...
- Gemini model names churn fast and free-tier quotas vary a lot per model (e.g. `gemini-2.5-flash` was 20 requests/day free, `gemini-2.5-flash-lite` got deprecated for new users mid-project). Default is the `gemini-flash-lite-latest` alias, which Google keeps pointed at their current recommended low-cost model — if you see a 404 NOT_FOUND on the configured model, run `client.models.list()` to see what's actually available to your key rather than guessing a name from docs/memory. Also: not every model accepts `thinking_config` (some 400 outright) — `app/llm.py:_gemini_generate` retries without it automatically, so this shouldn't need touching per-model.
