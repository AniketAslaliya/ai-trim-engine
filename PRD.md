...
- **Sequencing**: one LLM call (`compose.py`) sees every non-silent segment from every provided video, tagged with its source `video_id`, and returns the segments to use in final output order. This reuses the same tag data (`transcript`/`scene_tags`/`objects`/`action_tags`) the single-video pipeline already extracts — no new extraction work. Tag similarity biases *which* sequence gets picked (a cheap, always-available signal fed into the same LLM call).
- **Match cuts — real frame + audio analysis at each join, not a tag proxy** (`match_cut.py`): once a sequence is chosen, every cross-video join is re-examined for real. The actual last frame of the outgoing clip and first frame of the incoming clip are extracted with ffmpeg and sent to a vision LLM in one call (`llm.complete_multi_vision_json`) to judge visual continuity (composition/pose/framing) and return a 0–10 score + one-line reason. Separately, `ffmpeg`'s `volumedetect` filter measures the real mean audio level either side of the same boundary, and the two levels' delta (dB) is reported. Both numbers are genuine measurements on the actual boundary frames/audio — verified end-to-end against real footage to vary per join rather than return a constant. This is still **not** full frame-level CV match-cut detection: no optical flow, no tracked-object trajectory, no motion-vector comparison — it's a multimodal model's holistic judgment on two real frames plus a real loudness delta. If frame extraction or the LLM call fails, the join degrades to the tag-overlap count rather than fabricating a score. See §9a.
...
- **Match cuts use real frame/audio analysis, but still not true CV matching** — see §5b. `match_cut.py` extracts the actual two boundary frames at each cross-video join and sends them to a vision LLM for a real continuity judgment, plus measures a real ffmpeg audio-level delta — genuine analysis of the real footage, not a tag proxy. It is still not frame-level *CV* match-cut detection: no optical flow, no pose/skeleton tracking, no motion-vector comparison — it's a multimodal model's holistic visual judgment on two frames, which can miss or misjudge subtle motion-direction/composition matches a dedicated CV pipeline (or a human editor) would catch. Falls back to the cheaper tag-overlap count if frame extraction or the LLM call fails, rather than fabricating a score.
